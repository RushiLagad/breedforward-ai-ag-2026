...
| The advancement index is stable | 500 random weightings of a five-trait index keep a median 85% overlap with the default advancement set. Equal weights are the one choice that breaks it. |
...
| Sibling-informed prediction | `src/predict_siblings.py`, `src/predict_two_stage.py` | Half of each 2008 family known: family mean, markers plus siblings, within-family marker model, breeder's equation, advancement list |
...
| Demo | `dashboard/app.py` | Streamlit: move the index weights and the plot budget, see the advancement list and its realised 2008 gain |
