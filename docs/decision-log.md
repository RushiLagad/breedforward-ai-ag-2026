...
| Sep 19 | Sibling stages framed as an after-harvest decision on the untested siblings; January decision is the plot plan (sampling curve) | "January prediction" for everything | half of every family is phenotyped in stages 3 to 5; only the untested lines' outcomes are hidden | Rishi |
| Sep 19 | Stratified family split and training-only centering; ties break on LINE_ID | independent line sampling, centering on all plots | independent sampling left small families with no plots (curve at 10% was +3.9, now +4.5); centering on all plots let held-out outcomes into the cell means | Rishi |
| Sep 19 | Two-stage yield in the index although the sibling mean wins yield alone (0.364 vs 0.360) | sibling mean for yield | index gains tied (+4.15 vs +4.13); two-stage ranks within families and keeps 115 families in the flagged set vs 52; rule and both outcomes in stage3_index_choice.csv | Rishi |
